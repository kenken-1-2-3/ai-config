# R017 CMS 網站說明 責任博弈 `/webInformationCms/4` Spec
...
>
> **架構決議（已實作）**：本批四份網站說明改採**純動態路由** `/webInformationCms/:id`（對齊舊 set_r017），不再做 4 個固定 `/webInformation/ResponsibleGaming` wrapper。原因：與 set_r017 模板架構一致、後台新增 CMS 項目無需改 code、單一頁面覆蓋所有 web information 內容。固定 4 條 wrapper 已刪除。

## 背景 / 目標

把網站說明「責任博弈」從舊專案靜態內容改為 CMS-driven 內容，在 NX r017 透過動態 route `/webInformationCms/4` 提供。id 對應 `CMS_WEB_INFORMATION_URL_ID_ENUMS.RESPONSIBLE_GAMING = 4`，內容來源為 CMS `WEBSITE_INFORMATION = 5` 的對應 item。

需求列項：`CMS網站說明串接 責任博弈  /webInformationCms/4  ResponsibleGaming`。
...
2. 該頁從 `route.params.id` 取出 id，傳給 shared `CmsWebInformationPage`；URL 走 `/webInformationCms/4` 代表 ResponsibleGaming。
3. 對所有需要導到責任博弈的入口（`useStaticPageDirection("responsible")`）改用 `toWebInformationCmsRoute(CMS_WEB_INFORMATION_URL_ID_ENUMS.RESPONSIBLE_GAMING)` 產生 `/webInformationCms/4`。
...
- 不搬舊專案 `ResponsibleGaming.vue` 的硬編英文段落。
...
5. `useStaticPageDirection("responsible")` 經 `toWebInformationCmsRoute(RESPONSIBLE_GAMING)` 導到 `/webInformationCms/4`。
...
- [ ] `/webInformationCms/4` 可直接開啟，未登入也可進入。
- [ ] 頁面呼叫 CMS list API，request 帶 `type=CMS_TYPE_ENUMS.WEBSITE_INFORMATION`。
- [ ] 從 CMS list 中選到 `url_id === 4` 的 item；若沒有，才 fallback 到 `id === 4`。
...
- [ ] `useStaticPageDirection().handleStaticPageAction("responsible")` 導到 `/webInformationCms/4`。
...
4. `pnpm nx serve r017` 手動驗證 `/webInformationCms/4`、語系切換、窄 viewport 水平捲動。
5. 對照 Figma node `15100:154406` 做 desktop visual check。
6. `pnpm nx build r017`。

## Git Flow

- 分支名稱：`feat/cms-web-information-responsible-gaming`（或合併 4 份成 `feat/cms-web-information`）
