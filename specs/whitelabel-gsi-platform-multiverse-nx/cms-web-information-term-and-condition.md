# R017 CMS 網站說明 條款與條約 `/webInformationCms/2` Spec
...
>
> **架構決議（已實作）**：本批四份網站說明改採**純動態路由** `/webInformationCms/:id`（對齊舊 set_r017），不再做 4 個固定 `/webInformation/TermAndCondition` wrapper。原因：與 set_r017 模板架構一致、後台新增 CMS 項目無需改 code、單一頁面覆蓋所有 web information 內容。固定 4 條 wrapper 已刪除。

## 背景 / 目標

把網站說明「條款與條約」從舊專案靜態內容改為 CMS-driven 內容，在 NX r017 透過動態 route `/webInformationCms/2` 提供。id 對應 `CMS_WEB_INFORMATION_URL_ID_ENUMS.TERMS_AND_CONDITIONS = 2`，內容來源為 CMS `WEBSITE_INFORMATION = 5` 的對應 item。

需求列項：`CMS網站說明串接 條款與條約  /webInformationCms/2  TermAndCondition`。
...
2. 該頁從 `route.params.id` 取出 id，傳給 shared `CmsWebInformationPage`；URL 走 `/webInformationCms/2` 代表 TermAndCondition。
3. 對所有需要導到條款與條約的入口（`useStaticPageDirection("terms")`、login/register 既有 Terms 入口）改用 `toWebInformationCmsRoute(CMS_WEB_INFORMATION_URL_ID_ENUMS.TERMS_AND_CONDITIONS)` 產生 `/webInformationCms/2`。
...
- 不搬舊專案 `TermAndCondition.vue` 的硬編英文段落。
...
5. `useStaticPageDirection("terms")` 經 `toWebInformationCmsRoute(TERMS_AND_CONDITIONS)` 導到 `/webInformationCms/2`。
...
- [ ] `/webInformationCms/2` 可直接開啟，未登入也可進入。
- [ ] 頁面呼叫 CMS list API，request 帶 `type=CMS_TYPE_ENUMS.WEBSITE_INFORMATION`。
- [ ] 從 CMS list 中選到 `url_id === 2` 的 item；若沒有，才 fallback 到 `id === 2`。
...
- [ ] `useStaticPageDirection().handleStaticPageAction("terms")` 導到 `/webInformationCms/2`。
- [ ] login/register 既有 Terms 入口導到 `/webInformationCms/2`。
...
4. `pnpm nx serve r017` 手動驗證 `/webInformationCms/2`、語系切換、窄 viewport 水平捲動。
5. 對照 Figma node `15100:154406` 做 desktop visual check。
6. `pnpm nx build r017`。

## Git Flow

- 分支名稱：`feat/cms-web-information-term-and-condition`（或合併 4 份成 `feat/cms-web-information`）
